...
{
    // Check if the iOS version is 10.0 or above to use the new API
    if (@available(iOS 10.0, *)) {
        // Use the new method: openURL:options:completionHandler:
        [[UIApplication sharedApplication] openURL:url options:@{} completionHandler:^(BOOL success) {
            // Completion handler runs after attempting to open the URL
            if (!success) {
                [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:CDVPluginHandleOpenURLNotification object:url]];
            }
        }];
    } else {
        // Fallback for older iOS versions that don’t support the new method
        // Use the deprecated openURL: method
        if ([[UIApplication sharedApplication] openURL:url] == NO) {
            // If the URL failed to open, send the same notification
            [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:CDVPluginHandleOpenURLNotification object:url]];
        }
    }
}
